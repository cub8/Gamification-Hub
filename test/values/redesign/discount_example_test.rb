...
class Redesign::DiscountExampleTest < ActiveSupport::TestCase
  SEEDS = (1..25)
  ...
  def card
    Redesign::ItemCard.new(@item.reload,
                           ranks:  @story_group.ranks.by_threshold.to_a,
                           badges: @story_group.badges.kept.by_name.to_a,)
  end

  def examples
    SEEDS.map { |seed| Redesign::DiscountExample.new(card, random: Random.new(seed)) }
  end
  ...
  test 'it never names more than two badges' do
    ...
    assert_operator sizes.max, :<=, Redesign::DiscountExample::MAX_BADGES
  end
