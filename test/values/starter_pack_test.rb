...
class StarterPackTest < ActiveSupport::TestCase
  Pack = StarterPack
  ...
  test 'every preset glyph is one the entity actually offers' do
    Pack::KEYS.each do |key|
      ...
      assert_empty preset.ranks.map(&:icon_glyph) - Glyphs::RANK
      assert_empty preset.badges.map(&:icon_glyph) - Glyphs::BADGE
      assert_empty preset.items.map(&:icon_glyph) - Glyphs::ITEM
    end
  end
